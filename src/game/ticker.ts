...
function calcTick(this: Game): void
{
    { // 为暂停按钮计算渐变
    ...
    switch (this._animateStatus)
    {
        ...
        case 1:
        {
            const { chart, effects, judgement, functions, processors, sprites, render, _settings: settings } = this;
            const currentTime = (chart.music as any).currentTime - (chart.offset + settings.offset);

            for (let i = 0, length = chart.bpmList.length; i < length; i++)
            {
                const bpm = chart.bpmList[i];
            ...
            for (let i = 0, length = chart.judgelines.length; i < length; i++)
            {
                ...
                for (let x = 0, pLength = processors.judgeline.length; x < pLength; x++) processors.judgeline[x](judgeline, currentTime);
            }
            for (let i = 0, length = chart.notes.length; i < length; i++)
            {
                const note = chart.notes[i];
                note.calcTime(currentTime, render.sizer);
                for (let x = 0, pLength = processors.note.length; x < pLength; x++) processors.note[x](note, currentTime);
            ...
            {
                judgement.calcTick();
                for (let x = 0, fLength = functions.tick.length; x < fLength; x++) functions.tick[x](this, currentTime);
                ...
                if (settings.shader)
                {
                    ...
                    for (let i = 0, length = effects.length; i < length; i++)
                    {
                        ...
                        if (effect.isGlobal) render.stage.filters.push(effect.shader as any);
                        else render.gameContainer.filters.push(effect.shader as any);
                    }
                }
            }

            (sprites.progressBar as any).scale.x = (chart.music as any).progress * (sprites.progressBar as any).baseScaleX;
...
function calcGameAnimateTick(this: Game, isStart: boolean = true): void
{
    const _progress = (Date.now() - (isStart ? this._gameStartTime : this._gameEndTime)) / 1500,
        progress = (isStart ? 1 - Math.pow(1 - _progress, 4) : Math.pow(1 - _progress, 4));
    const sprites = {
        score: this.judgement.score.sprites!,
        chart: this.chart.sprites as any
    };
    ...
    // 背景图亮度
    if (this.chart.sprites.bg && (this.chart.sprites.bg as any).cover) (this.chart.sprites.bg as any).cover.alpha = this._settings.bgDim * progress;
    ...
    if (_progress >= 1)
    {
        if (isStart)
        {
            ...
            setTimeout(async () =>
            {
                (this.chart.music as any).play(true);
