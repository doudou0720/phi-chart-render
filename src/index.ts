...
const doms = {
    ...
    file : {
        chart: document.querySelector('select#file-chart') as HTMLSelectElement,
        music: document.querySelector('select#file-music') as HTMLSelectElement,
        bg: document.querySelector('select#file-bg') as HTMLSelectElement
    },
    settings: {
...
doms.startBtn.addEventListener('click', async () => {
    ...
    currentFile.chart.music = currentFile.music.source;
...
function PlayLikeYouNeverDidBefore(game: PhiChartRender.Game, currentTime: number): void
{
    ...
    game.chart.music = game.chart.music; // keep same
    (game.chart.sprites.info as Record<string, { text: string }>).songName.text = game.chart.info.name + ' (x' + Math.round(currentSpeed * 100) / 100 + ')';
}
